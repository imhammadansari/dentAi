...
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(CURRENT_DIR, "models", "BEST_model.pt")
EDSR_MODEL_PATH = os.path.join(CURRENT_DIR, "models", "EDSR_x2.pb")  # Path for optional deep-learning EDSR weights
...
def check_image_quality(img_gray):
    ...
    if w < 500 or h < 500:
        warnings.append("Low resolution detected — executing automatic EDSR & Lanczos4 super-resolution upscaling")
...
def upscale_smart(img_gray):
    """
    Applies EDSR Deep Learning Upscaling if the model file is present.
    Gracefully utilizes Lanczos4 high-fidelity interpolation as a robust fallback.
    """
    if os.path.exists(EDSR_MODEL_PATH):
        try:
            sr = cv2.dnn_superres.DnnSuperResImpl_create()
            sr.readModel(EDSR_MODEL_PATH)
            sr.setModel("edsr", 2)  # Multiplier factor of 2
            img_color = cv2.cvtColor(img_gray, cv2.COLOR_GRAY2BGR)
            upscaled_color = sr.upsample(img_color)
            return cv2.cvtColor(upscaled_color, cv2.COLOR_BGR2GRAY), "EDSR (2x) Deep Learning"
        except Exception:
            pass
            
    h, w = img_gray.shape[:2]
    upscaled = cv2.resize(img_gray, (w * 2, h * 2), interpolation=cv2.INTER_LANCZOS4)
    return upscaled, "Lanczos4 Filter"
...
def preprocess_xray(img_bgr):
    ...
    quality_info = {
        ...
        "upscaling_applied": "None (Resolution Adequate)"
    }

    if w_orig < 500 or h_orig < 500:
        img_gray, method = upscale_smart(img_gray)
        quality_info["upscaling_applied"] = method
        quality_info["upscaled_size"] = f"{img_gray.shape[1]}x{img_gray.shape[0]}"
...
async def predict(file: UploadFile = File(...)):
    ...
    # Print image health metrics to terminal logs
    print("\n--- [IMAGE OPTIMIZATION PIPELINE METRICS] ---")
    print(f"Original Size: {quality_info['original_size']}")
    print(f"Brightness Level: {quality_info['brightness']} | Contrast: {quality_info['contrast']}")
    print(f"Blur (Laplacian Variance): {quality_info['blur_score']}")
    if "upscaled_size" in quality_info:
        print(f"Upscaling Event: {quality_info['upscaling_applied']} -> {quality_info['upscaled_size']}")
    if "tilt_corrected" in quality_info:
        print(f"Deskew Adjustment: {quality_info['tilt_corrected']}")
    if warnings:
        print(f"Active Pipeline Warnings: {warnings}")

    # 2. Deep Vision Inference Evaluation
    results = model(processed_img, conf=0.25, iou=0.20)
    display_img = processed_img.copy()
    ...
    if len(results) > 0 and results[0].boxes is not None:
        for idx, box in enumerate(results[0].boxes, 1):
            ...
            # Terminal log execution for confidence values alongside attributes
            print(f"[{idx}] Target: {label.upper()} | Confidence: {conf * 100:.2f}% | BBox: [{x1}, {y1}, {x2}, {y2}]")
            ...
            # Custom Overlay Annotation Rendering Layer (Clean Label Only)
            box_color = color_map.get(label, (0, 255, 0))
            cv2.rectangle(display_img, (x1, y1), (x2, y2), box_color, 2)
            
            # Formatted text without confidence percentage
            tag_text = f"{label.split('_')[0].upper()}"
            (text_w, text_h), _ = cv2.getTextSize(tag_text, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
            cv2.rectangle(display_img, (x1, y1 - text_h - 6), (x1 + text_w + 6, y1), box_color, -1)
            cv2.putText(display_img, tag_text, (x1 + 3, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
    else:
        print("No structural features matched minimum clinical confidence standards.")
    print("--------------------------------------------\n")
