...
const buildPDF = (report, patientName) => {
    return new Promise((resolve) => {
        ...
        doc.fontSize(9).fillColor('rgba(255,255,255,0.7)')
            .text('Report ID: ' + report._id.toString(), margin, 78)
        ...
        if (!report.detections || report.detections.length === 0) {
            ...
            report.detections.forEach((d, i) => {
                ...
                doc.font('Helvetica').fontSize(9.5).fillColor(GRAY)
                    ...
                    .fillColor(DARK).text(expText, { width: contentWidth - 40, continued: false });
                y += cardH + 8;
            });
        }
        ...
        doc.font('Helvetica').fontSize(8.5).fillColor('rgba(255,255,255,0.85)')
            .text('This report is generated by DentAI and is intended for informational purposes only.', margin, y + 10, { align: 'center', width: contentWidth })
            .text('Always consult a qualified dental professional for diagnosis and treatment decisions.', margin, y + 24, { align: 'center', width: contentWidth })
            .text('DentAI Platform  -  Confidential Patient Record', margin, y + 38, { align: 'center', width: contentWidth });
...
exports.checkHasPreviousReport = async (req, res) => {
    try {
        ...
        const count = await ReportModel.countDocuments({ patientId, comparisonReport: { $in: [null, ''] } });
